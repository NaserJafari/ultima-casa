<?php
     ...
     $sql = "SELECT huizen.ID as HID,
                ...
                Straat,
                CONCAT(LEFT(Postcode, 4), ' ', RIGHT(Postcode, 2), ', ', Plaats) as Plaats,
                MAX(CONCAT('&euro; ', Bod)) AS HoogsteBod,
                MAX(Status) AS Status
           FROM huizen
      LEFT JOIN relaties ON relaties.ID = huizen.FKRelatiesID 
      LEFT JOIN biedingen ON biedingen.FKhuizenID = huizen.ID
      LEFT JOIN statussen ON statussen.ID = biedingen.FKstatussenID
          WHERE relaties.ID = $ridSession
      GROUP BY huizen.ID, StartDatum, Straat, Postcode, Plaats
      ORDER BY StartDatum";
     ...
     echo '
          ...
          <body>
               <div class="container">' .
                    InlogKop($ridSession, "Mijn Ultima Casa", 
                                         '<td>
                                               <button class="action-button" title="De makelaar een e-mail sturen."> 
                                                       <a href="maakmail.php?RID=' . $ridSession . '&FID=' . $ridSession . '&TID=' . $makelaar["MID"] . '">&#x2709;
                                               </button>
                                          </td>') .
                                                       ...
                                                       <table class="details">
                                                            <tr>
                                                                 ...
                                                                 <td class="button-column">
                                                                      <button type="submit" class="action-button" id="RID" name="RID" 
                                                                              value="' . $ridSession . '" title="Huizen filteren op deze voorwaarden.">Ok
                                                                      </button>
                                                                 </td>
                                                            </tr>
                                                       </table>
     ...
     {    echo                    '<tr>
                                        ...
                                        <td class="button-column">
                                             <button class="action-button" title="Contact opnemen met de verkoper.">
                                                  <a href="maakmail.php?RID=' . $ridSession . '&FID=' . $ridSession . '&TID=' . $tekoop["RID"] . '">&#x2709;
                                        ...
                                        <form action="relatieupd.php" method="POST">
                                             ...
                                             <div class="form-group">
                                                  <button type="submit" class="action-button" id="upd" name="upd" 
                                                          value="' . $ridSession . '" title="Mijn account aanpassen.">Wijzigen
                                                  </button>
                                                  <button class="action-button"><a href="relatie.php?RID=' . $ridSession . '" >Annuleren</a></button>
                                             </div>
                                             <div class="form-group">
                                                  <button class="action-button button-column" 
                                                          title="Je Ultima Casa account verwijderen!">
                                                       <a href="accountmin.php?ID=' . $ridSession . '">Account verwijderen</a>
                                                  </button>
                                             </div>
                                        </form>
                                   </div>
